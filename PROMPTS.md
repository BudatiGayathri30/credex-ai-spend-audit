...
- Early versions produced polished but occasionally overconfident language about savings certainty.
- Adding the explicit realism clause ("depends on rollout, seat sizing, and adoption") reduced exaggerated claims.
- Constraining response length to 90-120 words improved scannability in both dashboard and share-page UI.
- JSON-only output instruction significantly reduced parser failures from markdown wrappers.

## Failed prompt attempts

- **Attempt:** ask for "executive summary with bold recommendations."
  - **Failure mode:** model drifted into markdown formatting and broke JSON parsing.
- **Attempt:** include too many numeric instructions in one block.
  - **Failure mode:** responses became repetitive and occasionally missed the top recommendation.
- **Attempt:** omit fallback framing.
  - **Failure mode:** generated copy sounded absolute ("you will save..."), which is risky for trust.

## Hallucination prevention strategy

- Only pass structured recommendation data generated by deterministic engine logic.
- Prohibit personal data and non-existent fields in prompt instructions.
- Require strict output schema and parse with JSON validation.
